...
from search_orchestration.adapters.ai.taxonomy import MUSIC_TAXONOMY, SFX_TAXONOMY, get_taxonomy_summary_for_prompts

_TAXONOMY_SUMMARY = get_taxonomy_summary_for_prompts(
    MUSIC_TAXONOMY, max_examples=None)
_SFX_TAXONOMY_SUMMARY = get_taxonomy_summary_for_prompts(
    SFX_TAXONOMY, max_examples=None)
...
_HUMAN_TONE_SFX = (
    "Sound like a real person: natural, warm, concise. Short sentences. Use sound-friendly language (vibe, feel, style, sound). "
    "Never output long lists like 'Category: X; Subcategory: Y'. "
    "When mentioning filters, use at most 2–3 terms in a natural phrase. "
    "Do not use the word 'something' or vague phrases like 'things like'."
)

_EXPLAIN_SYSTEM_MUSIC = (
...
_EXPLAIN_SYSTEM_SFX = (
    "You are a helpful assistant explaining what is happening in the SFX search, step by step, "
    "in simple human language so the user can follow along. Write 1–3 short sentences. Be clear and friendly. "
    f"{_HUMAN_TONE_SFX} "
    "Our search uses only these filter categories and their allowed terms: "
    f"{_SFX_TAXONOMY_SUMMARY}. "
    f"{_CROSS_CHECK_RULES} "
    "Do not mention internal details like 'round', 'broaden', or JSON."
)
...
def get_explain_prompt_soundstripe_search() -> ChatPromptTemplate:
    ...
    return ChatPromptTemplate.from_messages([
        SystemMessagePromptTemplate.from_template(_EXPLAIN_SYSTEM_MUSIC),
        human_msg,
    ])
...
def get_explain_prompt_soundstripe_search_sfx() -> ChatPromptTemplate:
    human_msg = HumanMessagePromptTemplate.from_template(
        "The user asked for: {user_text}\n"
        "Filters we used: {filters_summary}\n\n"
        "We found {songs_count} sound effects; {new_songs_count} are new (added to their results).\n\n"
        "In 1–2 short sentences tell them we found {songs_count} sound effects and added {new_songs_count} new ones. "
        "Use the exact numbers. Optionally add a brief phrase with at most 2–3 terms from {filters_summary}. "
        "Do not use 'something' or 'things like'."
    )
    return ChatPromptTemplate.from_messages([
        SystemMessagePromptTemplate.from_template(_EXPLAIN_SYSTEM_SFX),
        human_msg,
    ])


def get_explain_prompt_record_debug_sfx() -> ChatPromptTemplate:
    human_msg = HumanMessagePromptTemplate.from_template(
        "The user asked for: {user_text}\n"
        "Filters we just used: {filters_summary}\n"
        "Sound effects this round: {last_round_count}. Total so far: {total_results}. Target achieved: {target_achieved}. Will loop: {will_loop}.\n\n"
        "If target_achieved is True: Say we have enough sound effects and we're presenting the results. "
        "If target_achieved is False and will_loop is True: Say we'll broaden the filters and fetch more sound effects. "
        "If target_achieved is False and will_loop is False: Say this is what we found and we're presenting the results. "
        "Use at most 2–3 terms from {filters_summary} when mentioning filters. Do not use 'something' or 'things like'."
    )
    return ChatPromptTemplate.from_messages([
...
def get_explain_prompt_finish_sfx() -> ChatPromptTemplate:
    human_msg = HumanMessagePromptTemplate.from_template(
        "The user asked for: {user_text}\n"
        "Search is complete. We found {total_results} sound effects.\n\n"
        "In one short sentence say search is complete and we're presenting the results."
    )
    return ChatPromptTemplate.from_messages([
        SystemMessagePromptTemplate.from_template(_EXPLAIN_SYSTEM_SFX),
...
def get_explain_prompt_plan_round_sfx() -> ChatPromptTemplate:
    """Single combined explain for plan_round: announce + explain_strategy + generate + validate in one message."""
    human_msg = HumanMessagePromptTemplate.from_template(
        "The user asked for: {user_text}\n"
        "We just completed the planning step: we turned their request into filters and merged them. "
        ...
        "1) If is_first_round: briefly say we're starting their search, then that we've turned their request into filters. "
        "2) If not is_first_round: say we're running the search again with broader filters, then that we've got new filters. "
        "3) Mention the filters using at most 2–3 terms from {filters_summary} in a natural phrase (e.g. that cinematic, explosion vibe). "
        "4) Say we're about to search the catalog with these filters. "
        "Keep it human and concise. Do not use 'something' or 'things like'. No long Category: Term lists."
    )
    return ChatPromptTemplate.from_messages([
...
EXPLAIN_PROMPTS_SFX = {
    "plan_round": get_explain_prompt_plan_round_sfx(),
    "soundstripe_search": get_explain_prompt_soundstripe_search_sfx(),
    "record_debug": get_explain_prompt_record_debug_sfx(),
    "finish": get_explain_prompt_finish_sfx(),
}
